package com.xin.sparrow.common.aop.check;
...
public class CheckDataAop {
    ...
    @Pointcut("execution(* com.xin.sparrow..*.*(@com.xin.sparrow.common.annotation.check.CheckData (*), ..))")
    public void pointCut(){}

    @Before("pointCut()")
    public void begin(JoinPoint joinPoint) throws ExceptionMessageException, IllegalAccessException {
        ...
        for (int i = 0; i < annotations.length; i++) {
            ...
            for (Annotation annotation : paramAnn) {
                if(annotation.annotationType().equals(CheckData.class)){
                    if(param == null){
                        throw new ExceptionMessageException("[" + method.getName() + "]方法参数为空");
                    }
                    ...
                    for (Field field : fields) {
                        ...
                        if(annotationPresent) {
                            ...
                            if (StringUtils.isEmpty(field.get(param))){
                                throw new ExceptionMessageException("[" + field.getName()  + "]字段为空");
                            }
                        }
                    }
                }
